...
import routes from "../routes/magang";
...
const DashboardMagang = ({ history }) => {
  const redirectlogin = useHistory();
  const handleRedirect = () => {
    redirectlogin.push('/signin')
  }
  ...
  const getRoutes = (
    ...
    <Switch>
      ...
      <Route
        exact
        path='/magang/ubah_password'
        component={UbahPassword}
        name='Ubah Password'
      />
      <Redirect to="/404" />
    </Switch> : handleRedirect()
  );
