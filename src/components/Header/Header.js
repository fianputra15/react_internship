...
const Header = ({
  ...
  const handleRedirectUbahPassword = () => {
    history.push('/magang/ubah_password');
  }
  const handleCloseMenu = () => setAnchorEl(null);
  const redirect = useHistory();
  const handleUbahPassword = () => {
    handleRedirectUbahPassword();
  }
  const handleSignOut = () => {
    removeLocalStorage()
    ...
    <AppBar position="static" className={classes.appBar}>
      <Toolbar className={classes.toolBar}>
        ...
        <div className={classes.branding}>
          <Typography gutterBottom variant = "h6" component = "h6"  >InternApps</Typography>
        </div>
        <Hidden xsDown>
        ...
        >
          <MenuItem onClick={handleUbahPassword}>
            <ListItemIcon>
              <SettingsIcon />
            </ListItemIcon>
